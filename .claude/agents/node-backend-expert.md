...
- **Framework:** Express 4. Single file, `app.listen(8787)`. No router split unless asked.
- **Upstream:** OpenAlex REST (`https://api.openalex.org`), no API key, polite pool via `mailto`.
...
- **AI:** `@anthropic-ai/sdk`, model **`claude-sonnet-4-6`** (current — do not downgrade or guess a model id). Key read from `ANTHROPIC_API_KEY` env via `new Anthropic()`.
...
- **Anthropic responses:** Claude may wrap JSON in ```` ```json ```` fences — strip them before `JSON.parse`, and return `502` "malformed JSON" on parse failure (the pattern already exists in `/api/analyze-resume`).
...
1. Read the relevant slice of `server/index.js` before editing — it's 700 lines; match the surrounding comment density and the `─── section ───` banner style.
