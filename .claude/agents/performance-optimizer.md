...
1. **OpenAlex round-trips.** Every `oaFetch` miss is a network call. Levers:
   ...
   - **Typeahead routes** (`/api/institutions` → OpenAlex, `/api/schools` → Wikidata via `wdFetch`) fire per keystroke from the frontend — confirm they're debounced client-side and that their 10-min URL cache is actually hitting, or they become the dominant request volume.
2. **Anthropic calls** — two routes, two models: `/api/analyze-resume` uses **`claude-haiku-4-5`** (resume vision/PDF extraction — already the cheap/fast model; don't "upgrade" it to Sonnet for quality without being asked, that's a cost regression); `/api/professor/:authorId/draft-email` uses **`claude-sonnet-4-6`** (prose). Levers: `max_tokens` sizing, prompt length, and not re-calling on retryable client errors. Confirm the model id before switching anything.
